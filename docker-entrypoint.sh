...
#init ssl
docker_init_ssl() {
    if [ -z "$POSTGRES_SSL_KEY" ] && [ -z "$POSTGRES_SSL_CRT" ]; then
    	cat >&2 <<-'EOWARN'
			********************************************************************************
    	...
    	cat >&2 <<-'EOWARN'
			********************************************************************************
			USING: POSTGRES_SSL_KEY and POSTGRES_SSL_CRT from env
			********************************************************************************
		EOWARN
        cd
...
_main() {
	...
	if [ "$1" = 'postgres' ] && ! _pg_want_help "$@"; then
		...
		if [ ! -f "/var/lib/postgresql/data/INSTALLED" ]; then
			docker_verify_minimum_env
            ...
            echo "ssl_ciphers = 'HIGH:MEDIUM:+3DES:!aNULL'" >> postgresql.conf
            echo "ssl_prefer_server_ciphers = on" >> postgresql.conf
			unset PGPASSWORD
			...
			EOM
			echo "1" > /var/lib/postgresql/data/INSTALLED
		else
			cat <<-'EOM'
